...
| **PR ready-approved delivery** | `kind = 'pr_ready_approved'` is emitted for Quay-owned tasks when the task reaches `done` and the latest review-only attempt for the current `head_sha` has `review_verdict = 'approved'`. It covers both orderings: approval first with later CI pass, and CI pass first with later approval. The delivery payload contains `task_id`, `external_ref`, `repo_id`, `pr_number`, `pr_url`, `head_sha`, `review_id`, `review_attempt_id`, and `branch_name`. The route hint contains `slack_thread_ref` and fallback `deployment_default_slack_channel`; Hermes posts to the recorded thread when present and otherwise uses the deployment default Slack channel. |
