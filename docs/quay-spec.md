...
- **Orchestrator (Hermes):** owns business context — fetches tickets, gathers surrounding context, classifies complexity, composes briefs, talks to Linear, decides *when* to escalate to a human and *which* Slack thread to escalate into.
...
```
     ...
     └────▲───┘             └────┬────┘              └────┬────┘            └──┬───┘
          ...
          │                      ▼
          │            ┌─────────────────────┐
          │            │ awaiting-next-brief │ ◄── waiting_human reply ingested
          │            └──────────┬──────────┘     claim timeout
...
| `waiting_human` | Slack escalation posted; awaiting reply. | No | Poll Slack thread for non-bot reply. On reply: ingest, transition → `awaiting-next-brief`. |
...
- Non-consuming respawn reasons: `submit-brief --reason advice_answered` (after a `waiting_human` reply), `review` (CHANGES_REQUESTED), `conflict` (merge conflict).
...
for each task in active states:
  ...
  switch task.state:
    ...
    case waiting_human:
      # Tick is the SOLE Slack writer for this task — see §5 "Slack post (escalate-human)".
      # The CLI command only persists the artifact + transition; everything below runs
      # under the tick lock, so there is no race with another Slack writer.
...
- **Slack post (escalate-human): tick is the single owner.** Slack posting and reply ingestion are owned exclusively by tick, under the tick lock. The CLI command `quay escalate-human` only persists the artifact and transitions state — it never calls the Slack API. This makes Slack posting single-writer by construction and eliminates the prior race where a tick could re-post during the CLI's post-and-record window.

  **Sequence A — `quay escalate-human` (CLI, synchronous, no Slack API):**

  1. **Mint the escalation identity (SQL).** Within one SQL transaction: validate the ownership fence (`claim_id`, `cancel_requested_at IS NULL`); increment `tasks.next_escalation_seq` to assign this escalation a sequence number; generate an opaque `escalation_nonce` (a random URL-safe string, ≥ 16 bytes of entropy, formatted as `quay-esc-<task_id_short>-<seq>-<random8>` so it's both human-recognizable and uniquely searchable in Slack). The nonce is per-escalation, not per-task — a second escalation gets a different nonce.
  2. **Persist artifact + transition (same SQL transaction).** Write `slack_escalation_post` artifact row with `escalation_seq` set, `escalation_nonce` set, `content_hash` set (over `question_body || escalation_seq || escalation_nonce`), `slack_pre_post_fence_ts = NULL` (the fence is captured by tick, not the CLI — see Sequence B), `slack_post_ts = NULL`, `slack_recovered_post_ts = NULL`. Transition `tasks.state = 'waiting_human'`, clear `claim_id`, write `human_escalated` event. Resolve `slack_thread_ref` (CLI flag → task's enqueue-time default → error). Commit.
  3. The CLI returns success. The task is now visible in `waiting_human`; the orchestrator can move on. Slack posting is the next tick's responsibility.

  **Sequence B — tick `waiting_human` handler (the only Slack writer):** runs under the tick lock, so it cannot race with itself; it cannot race with the CLI either, because the CLI no longer posts. Per tick cycle, for each task in `waiting_human`:

  1. Read the latest `slack_escalation_post` artifact for the task. If `slack_post_ts IS NOT NULL AND slack_recovered_post_ts IS NOT NULL`, jump to step 5 (poll for replies).
  2. **Read the pre-post fence (Slack API).** If `slack_pre_post_fence_ts IS NULL`, call `conversations.replies` on `slack_thread_ref` and capture the latest `ts` as `<fence>`. Persist `slack_pre_post_fence_ts = <fence>` in one SQL transaction. (If the read fails, log `tick_error` and skip; next tick retries.) Capturing the fence at this point — *just before* the post — is what bounds the reply-eligibility risk window.
  3. **Try to recover an existing post via the nonce.** Call `conversations.replies` (or `search.messages`, depending on Slack workspace policy) and look for a bot-authored message whose body contains the artifact's `escalation_nonce`. If found: set `slack_recovered_post_ts` (and `slack_post_ts` if NULL) to the matched ts in one SQL transaction. This handles the case where a previous tick posted but crashed before persisting the ts — the recovery resolves purely by reading, with no duplicate post.
  4. **Post if no recovery match.** Only when `slack_post_ts IS NULL AND slack_recovered_post_ts IS NULL` after step 3, post to Slack. Body is `<question_body>\n\n_<escalation_nonce>_` (italic footer, searchable). On success: set `slack_post_ts` and `slack_recovered_post_ts` to the returned ts in one SQL transaction. On failure: log `tick_error` and skip; next tick retries (step 3 will recover if Slack actually accepted the post but the response was lost).
  5. **Poll for replies.** Lower bound is `slack_recovered_post_ts` if non-NULL, else `slack_pre_post_fence_ts`. Ingest the first non-bot reply with `ts > lower_bound` as a `slack_reply` artifact and transition to `awaiting-next-brief`.

  Once `slack_recovered_post_ts` is non-NULL, it is **never updated** — the first recovered ts wins, ensuring the cursor only moves forward.

  **Why this design is safe:**

  - **Single Slack writer.** Only tick posts. Tick acquires the process lockfile at start, so no two ticks race; the CLI is a non-writer for Slack, so it cannot race with tick.
  - **Crash-recovery is closed.** A tick that posts but crashes before persisting the ts is recovered by the *next* tick's nonce search at step 3 — `slack_recovered_post_ts` is filled from the existing Slack message, no duplicate post happens.
  - **Per-escalation idempotency.** The recovery-path partial unique index `(task_id, attempt_id, kind, content_hash)` includes `escalation_seq` and `escalation_nonce`. A crash-recovery retry of step 2 of Sequence A reuses the same row; a legitimate second escalation mints fresh seq + nonce and writes a distinct row.
  - **Latency bound.** The post lands within one tick interval (default 5 min) of `escalate-human` returning. Acceptable for human-in-the-loop work; in practice the post is visible within seconds because the next tick fires soon.
  - **Repost body.** Verbatim, including the nonce footer. Identical across recovery retries of the same escalation, so the nonce search is unambiguous.
...
| **Human-required** | Orchestrator decides the blocker needs a human (calls Quay to transition to `waiting_human`) | Slack post + reply ingestion. Reply pushed to orchestrator → next brief via `submit-brief`. | Does NOT consume |
...
- **Non-budget respawn dedup.** Review feedback and merge conflict respawns don't consume budget, but they must not loop on the same observation. Quay records what it last acted on per task:
  ...
  - `advice_answered` is naturally dedupe-safe: each Slack reply produces exactly one ingestion via the artifact-store crash-safe path.
...
| Human-required | Quay assembles the Slack post; the resulting reply is folded into the next brief by the orchestrator |
...
| `slack_escalation_post` | Quay posts to Slack on escalation | Quay's outbound message body + thread ref | File |
...
CREATE TABLE tasks (
  ...
  slack_thread_ref TEXT,              -- channel + ts, populated when escalation is posted
  claimed_at TEXT,                    -- set on quay task claim; cleared on submit-brief / escalate-human / release-claim / claim timeout / terminal transition.
  claim_id TEXT,                      -- opaque ownership token (UUID v4) returned by `quay task claim` and required by every subsequent claim-scoped write. Set atomically on each successful claim; cleared whenever the claim ends (release-claim, submit-brief, escalate-human, claim timeout, or terminal transition). A new claim mints a fresh value, fencing out any stale claimant whose previous claim was timed out and re-claimed by someone else.
  ...
  next_escalation_seq INTEGER NOT NULL DEFAULT 1,  -- monotonic; minted (and incremented) inside the step-1 SQL txn of the Slack post sequence (§5), alongside the per-escalation nonce. Distinguishes a legitimate second escalation from a recovery-path retry.
...
CREATE TABLE artifacts (
  ...
  escalation_nonce TEXT,              -- only set on slack_escalation_post artifacts; opaque per-escalation identifier embedded in the Slack post body (e.g. "quay-esc-<task_id_short>-<seq>-<random8>"). Used by reply ingestion to recover the bot-post ts via Slack search.
  slack_pre_post_fence_ts TEXT,       -- only set on slack_escalation_post artifacts; the thread's latest ts as read by tick in Sequence B step 1 of §5 "Slack post (escalate-human)". The CLI does NOT capture the fence — tick does, just before posting. NULL until tick fills it. Used as the *fallback* reply-cursor lower bound until slack_recovered_post_ts is filled.
  slack_post_ts TEXT,                 -- only set on slack_escalation_post artifacts; NULL until step 5 of the Slack post sequence completes successfully. Recorded for forensics/display.
  slack_recovered_post_ts TEXT,       -- only set on slack_escalation_post artifacts; the bot-post ts as observed in Slack (either returned in step 5, or recovered later by searching the thread for the escalation_nonce). The canonical reply-cursor lower bound when non-NULL. Set exactly once and never updated.
...
| `quay escalate-human <task_id> --claim-id <claim_id> --question-file <path> [--thread-ref <channel:ts>]` | Transition a task to `waiting_human`. **Requires `--claim-id` matching the live `tasks.claim_id`** (ownership fence) and `cancel_requested_at IS NULL`; same error taxonomy as `submit-brief`. **The CLI does NOT post to Slack** — it only persists the `slack_escalation_post` artifact (with `escalation_seq`, `escalation_nonce`, `content_hash` set; `slack_pre_post_fence_ts`, `slack_post_ts`, and `slack_recovered_post_ts` all NULL — tick captures the fence and posts) and transitions to `waiting_human` in one SQL transaction. Slack posting, fence read, and reply ingestion are owned exclusively by tick under the tick lock; see §5 "Slack post (escalate-human)" for why. Thread resolution: `--thread-ref` if provided; else the task's `slack_thread_ref` set at enqueue; else error. The chosen ref is persisted on the task. Clears `claim_id` and resets `claim_expirations_consecutive` to 0 on success. Does not consume budget. **Latency note:** the actual Slack post lands within one tick interval (default 5 min). | Orchestrator |
...
- **Tick is the single Slack writer.** All Slack API calls (post, fence-read, nonce-recovery, reply poll) happen inside `quay tick` under the supervisor lockfile. The CLI commands (`escalate-human`, `cancel`) never call the Slack API. This is what makes Slack posting safe under crashes and concurrent operator commands — there is exactly one writer process, serialized by the supervisor lock. (`quay cancel` also acquires that lock before running its finalizer, so it cannot race with a tick that's mid-Slack-post on the same task.)
- On `quay escalate-human`, Quay persists the question as a `slack_escalation_post` artifact and transitions the task to `waiting_human` in one SQL transaction. The thread ref (channel + message ts) is taken from `--thread-ref` on the command, or from the task's `slack_thread_ref` (set at enqueue time via `--slack-thread-ref`). If neither is available, `escalate-human` errors. The chosen ref is persisted on the task. **No Slack API call happens during the CLI command.** The next `quay tick` performs the actual post (see §5 "Slack post (escalate-human)" for the full sequence).
- During `waiting_human`, tick performs (under the lock, in order): pre-post fence read if not yet captured, nonce-based recovery search for an existing bot post, post-if-needed, reply poll. The reply lower bound is `slack_recovered_post_ts` when known, else the fallback `slack_pre_post_fence_ts` — using the recovered ts means thread chatter that lands between Quay's decision-to-escalate and the visible post is never eligible to be the answer. The first eligible non-bot reply is recorded as a `slack_reply` artifact and the task transitions to `awaiting-next-brief`.
- If Slack is down (rate limit, auth fail, network timeout), tick logs the error per-task (`tick_error`) and continues. The next tick retries. Latency budget: the post lands within one tick interval (default 5 min) of `escalate-human` returning.
- Auth, retries, rate-limit handling, credential rotation: implementation detail, not specified here.
- The orchestrator picks the thread (typically the originating ticket's Slack thread) and passes the ref. Quay does not decide where to post.
...
11. **Orchestrator claims, then calls `quay escalate-human`.** Asserts: claim transition, `slack_escalation_post` artifact written with `escalation_seq` and `escalation_nonce` set and all three timestamp fields (`slack_pre_post_fence_ts`, `slack_post_ts`, `slack_recovered_post_ts`) NULL, transition to `waiting_human`, `claim_id` cleared, **no Slack API call made by the CLI**. The next `quay tick` performs the fence read, posts to Slack, and fills in the timestamps.
11a. **Single Slack writer (no duplicate posts under operator/tick concurrency).** Setup: a wedge that fires `quay escalate-human` and `quay tick` at the same wall-clock time (the operator could realistically do this with a cron-driven tick). The CLI commits the artifact + transition; the tick runs under the lockfile and performs the post. Asserts: exactly one Slack post is created across both processes' lifetimes; `slack_post_ts` is set exactly once; the recovery-path partial unique index does not fire (the artifact row is created exactly once with one content_hash). This is the regression boundary for the prior design's race window.
11b. **Tick is the only Slack poster across crashes.** Inject a process kill on the tick handler immediately after the Slack API call returns ts1, before the SQL persist of `slack_post_ts`. Asserts: next tick's nonce-recovery search finds the bot post by `escalation_nonce` and sets `slack_recovered_post_ts = ts1` without re-posting; only one Slack post visible.
12. **Tick polls `waiting_human`, ingests a Slack reply.** Asserts: reply artifact, transition to `awaiting-next-brief` (no push; orchestrator picks up via its own pull).
