...
|---|---|---|
| `quay enqueue --repo <id> --brief-file <path> --ticket-snapshot-file <path> [--base-branch <branch>] [--external-ref <id>] [--slack-thread-ref <channel:ts>]` | Register a new task in `queued`. **Synchronously bootstraps the worktree:** validates the bare clone exists at `<repos_root>/<repo_id>.git` (operator-materialized; quay does not clone), runs `git fetch origin <effective_base_branch>`, creates the worktree via `git worktree add`, runs `install_cmd`. `--base-branch` overrides the repo default for this task only. Snapshots the ticket and captures the brief. Optionally records a default Slack thread for future escalations. **Does not spawn the worker.** The next `quay tick` promotes the task to `running` when capacity allows. Errors and aborts (no task created) if any bootstrap step fails. | Orchestrator |
...
| `quay outbox list [--status <status>] [--task <task_id>] [--kind <kind>] [--handler-class <workflow_intervention\|delivery>] [--include-ineligible]` | List durable outbox items for orchestrator-owned side effects. Workflow/intervention items are the canonical backing rows for existing human-advice handoffs. Delivery items are notification-only work and do not claim or block task progress. Defaults to eligible `pending` rows. | Hermes / orchestrator |
| `quay outbox claim <outbox_item_id> [--claim-id <claim_id>]` | Claim one eligible pending outbox item. If `--claim-id` is omitted, Quay mints one. Delivery-only claims affect only the outbox row. Workflow handoffs remain compatible with `quay task claim` for task-level ownership fencing. | Hermes / orchestrator |
| `quay outbox complete <outbox_item_id> --claim-id <claim_id>` | Complete a claimed outbox item. For delivery-class items Quay also stamps `delivered_at`; for workflow/intervention items existing task-resume commands may complete the backing outbox row through the handoff compatibility wrapper. | Hermes / orchestrator |
| `quay outbox fail <outbox_item_id> --claim-id <claim_id> --error <message> [--next-eligible-at <iso>]` | Record `last_error`, clear the claim, and reopen the item as `pending` for retry, optionally after a cooldown. Idempotency remains enforced by Quay's `outbox_items.idempotency_key`, not by Slack or other side effects. | Hermes / orchestrator |
...
| **Shared delivery outbox** | All Quay-originated orchestrator side effects are represented by `outbox_items`. `handler_class = 'workflow_intervention'` backs task-affecting human/advice handoffs; `handler_class = 'delivery'` is for notification-only work that can be claimed, completed, failed, and retried without changing task state. Quay owns `idempotency_key` uniqueness, so downstream delivery systems do not have to infer duplicate emissions from Slack state. |
