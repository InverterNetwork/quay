...
## High-miss lenses (verified from this reviewer's own miss history)

These are the defect classes human reviewers most often found in commits this reviewer had already approved at the same SHA. Walk each lens deliberately before approving; if the PR touches the area, say what you checked.

1. **Interleavings.** For any stateful flow with external side effects (payments, proposals, notifications, infra operations): what happens on a crash between two steps, a retry of any step, or two concurrent actors? Success must be recorded only after the external system durably accepted the operation; a computed value is not proof of acceptance. Look for claim-before-send, missing recovery paths for stuck intermediate states, and stale-data windows (reorgs, watermarks, moving defaults under cursors).
2. **Deployed reality.** Does the change actually run end-to-end in the deployed shape? Routes registered in the route table, env vars/config plumbed through IaC into the runtime the code reads, referenced APIs/fields/commands actually exist, handlers scheduled and stacks cut over. Removing infra code does not remove already-deployed infrastructure.
3. **Failure paths.** Trace each new failure mode to where it lands: swallowed errors, failed side effects that report success, alarms that can never fire, UI success states on failed operations, and error classifications that bypass observability (e.g. only ≥500 counted as operational).
4. **Test quality.** Could each new or changed test ever fail? Flag env-guarded tests that pass vacuously outside that env, assertions that only cover one direction, dead test scaffolding, and CI jobs that can go green while running zero checks.
5. **Parallel sites.** Was the fix or pattern applied to every sibling site — both entrypoints, every route/handler, mint *and* redeem? A change that copies an existing template must carry all of the template's obligations, not just the visible ones.
6. **Units and boundaries.** On money-adjacent or numeric code: hardcoded token/currency decimals, big integers routed through floating point, silent scale changes (100x/1e12), null-vs-unset conflation, empty/zero handling, and defaults that move under stored data.
7. **Consumers.** For changes to shared primitives, response shapes, or events: enumerate the consumers (pinned schemas, sibling apps, legacy params) and verify none silently breaks; renames need aliases or a migration.
8. **Hot paths.** Unbounded external reads (no limit/pagination) or pathological complexity introduced on a request path.
...
- **Dead code** (flag as Non-blocking): unused exports, dead branches, duplicate implementations of existing behavior, and leftover scaffolding shipped with the change. When a PR replaces a path, the superseded path should be deleted, not kept alongside.
- **Redaction boundaries** (Blocking when secrets/PII are involved): any new data flowing into logs, error details, prompts, or synthesized templates is an exfiltration surface. Verify new log/error fields pass the repo's existing redaction or allowlist layer (key-name redactors miss aliased keys). A suppressed or silenced security warning (linter, scanner) counts as a finding unless its written justification actually holds.
