...
> **⚠️ SUPERSEDED — DO NOT USE AS A REFERENCE FOR CURRENT WORK.**
>
> This draft modelled review as a **side-attempt** hung off Quay-owned tasks (review-only attempts that never transition the task) and as a small synthetic-task state machine for human PRs. That framing is being replaced by a model where **PR review is a first-class state on the task itself**, entered by Quay-owned tasks after PR open and by synthetic tasks via an external trigger.
>
> The replacement spec is being drafted at `docs/quay-spec-pr-review.md` (TBD). Until it lands, this archive copy is kept for the still-valid design work it captured — dispatch shape, synthetic `task_id` identity scheme, `(task_id, head_sha)` dedup, `review_findings` table + FTS5, the `quay-principle` fenced-block contract, force-push handling, v1/v2 graduation. The parts that are stale are concentrated in §2 ("fire-and-forget, never gate"), §4 (state machine), and §8 (rationale for keeping review off the task lifecycle).
>
> If anything below conflicts with the new spec, the new spec wins.

**Status:** Superseded (was: Draft, not locked). Companion spec to `docs/quay-spec-ticket-validation.md`; second feature spec graduating from `docs/orchestrator-design-notes.md`.
