// AST-102: pin the gh `pr view --json` field set to portable values that
// work on gh 2.45.0 (baseRefOid was added in 2.46 — including it here makes
// every poll on older gh installs fail with "Unknown JSON field"). The
// adapter must request `baseRefName` instead and resolve it to a SHA via a
// `git merge-base origin/<baseRefName> <headRefOid>` shell-out, plus carry
// `number` + `url` so the operator-visible task row gets PR linkage.
